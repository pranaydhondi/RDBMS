...
public:
	...
	//			cout<<"next level\n";
                if(isLesser==0)handleNonLeaf(&current, i+1);
   ...
   //             current->display(keytype);
	...
	/*
	 * self added method to compare equality of keys
	 */
	 int isEqual(char* key1,char* key2,KeyType keytype){
		 int case1 = compare(key1,key2,keytype);
		 int case2 = compare(key2,key1,keytype);
    ...
    /*
     * custom split is used when we have dupicates
     * */
    int customSplit(TreeNode *node, TreeNode *newLeaf, int tempSpaceSize, char *tempSpace){
		int size = node->numkeys;
		int splitPos=0;
		int half = (size-1)/2;
		char nodekey1[keylen(&keytype)];
		char nodekey2[keylen(&keytype)];
		Utils::copyBytes(nodekey1,&(tempSpace[(half * keylen(&keytype))]),keylen(&keytype));
		Utils::copyBytes(nodekey2,&(tempSpace[((half +1) * keylen(&keytype))]),keylen(&keytype));
		//node->getKey(keytype, nodekey1, half);
		//node->getKey(keytype, nodekey2, half+1);
		if(isEqual(nodekey1,nodekey2,keytype)!=1){
			splitPos = half+1;
			doSplit(node,newLeaf,tempSpaceSize,tempSpace,splitPos);
		}else{
			int j;
			Utils::copyBytes(nodekey2,&(tempSpace[((size-1) * keylen(&keytype))]),keylen(&keytype));
			//node->getKey(keytype, nodekey2, size-1);
			if(isEqual(nodekey1,nodekey2,keytype)!=1){
				for(j=half+2;j<size-1;j++){
					Utils::copyBytes(nodekey2,&(tempSpace[(j * keylen(&keytype))]),keylen(&keytype));
					//node->getKey(keytype, nodekey2, j);
					if(isEqual(nodekey1,nodekey2,keytype)!=1)break;
				}
				splitPos = j;
				doSplit(node,newLeaf,tempSpaceSize,tempSpace,splitPos);
			}else{
